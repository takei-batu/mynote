...
$ 
  cal(N)[|vzero|] & = zero \
  cal(N)[|vone|] & = one \
  cal(N)[|vzero med n|] & = 2 dot cal(N)[|n|] \
  cal(N)[|vone med n|] & = 2 dot cal(N)[|n|] + 1 \
$
ここで，
$zero$と$one$は数学的な数（整数）である．
...
#Eg[
  上の意味関数$cal(N)$を使って数値$vone vzero vone$に対応する数$cal(N)[|vone vzero vone|]$を計算している．
  #note[
    accordance : 準拠 \
...
#Ex[
  ...
  を以下のように定める：
  $ 
    cal(L)[|vzero|] = cal(L)[|1|] & = one \
    cal(L)[|vzero med n|] = cal(L)[|vone med n|] & = cal(L)[|n|] + one \
  $
  これはwell-defindであり，
  ...
  $ 
    cal(N)[|vzero|] & = zero \
    cal(N)[|vone|] & = one \
    cal(N)[|vzero med n|] & = cal(N)[|n|] \
    cal(N)[|vone med n|] & = 2^(cal(L)[|n|]-1) + cal(N)[|n|] \
  $
  これもwell-defindであり，
...
#Fact[
  ...
  #proof[
    ...
    数値$n$が与えられたならば，
    それは4つの形のうちの1つ，
    すなわちそれが基本要素(a basis element)のときは$vzero$または$vone$，
    複合要素(a composite element)のときは別の数値$n'$が存在して$n' vzero$または$n' vone$と表せる．
    よって(@fact_1.5)を示すために，
    ...
    基底ケースでは，
    $Num$の基本要素，
    つまり$n$が$vzero$または$vone$の場合について示す．
    帰納ステップでは，
    $Num$の複合要素，
    つまり$n$が$n' vzero$または$n' vone$の場合について示す．
    ...
    それは$cal(N)[|n|] = zero$を与える．
    よって明らかに唯一の数$n in Int$（すなわち$zero$）が存在して$cal(N)[|n|] = bold("n").$
    ...
    ここで$n'$に帰納法の仮定を適用すると，
    $cal[|n'|] = bold("n'")$を満たす$bold("n'")$が唯一つ存在する．
    しかしこのとき，
    明らかに$cal(N)[|n|] = bold("n")$を満たす$bold("n")$（すなわち$2 dot n'$）が唯一つ存在する．
...
*構成的定義*は以下の要件を満たす。
+ 統語範疇は基本要素と複合要素を与える抽象構文によって記述される．
...
#note[
  compositional（構成的）は全体の意味を規定するには部分の意味を規定すればよいということを強調．
...
引数は一度にひとつずつとる．
例えば，$vx + vone$を第一引数にとると，
関数
$
  cal(A)[|vx + vone|] : State -> Int
$
を調べることになる．
状態を与えたときしか我々は$vx + vone$の値を取得できない．
...
#Eg[
  $s vx = three$の場合における$cal(A)[|vx + vone|]s$の計算を示している．
]
...
#Eg[
  ...
  $cal(A)$の定義に
  $
    cal(A)[|-a|]s = zero - cal(A)[|a|]s
  $
  をつけ加えればよい．
  ただし，
  $
    cal(A)[|-a|]s = cal(A)[|vzero - a|]s
  $
  は構成的定義の要求に矛盾する．
  ...
  #note[
    ...
    つまり，右辺に現れる$vzero$が左辺に現れる$-a$の直接の構成要素ではないということである．
    なお$-a$を$vzero - a$の省略と定義すれば特に問題はない．
...
#Ex[
  $cal(A)[|x|]s = s x = three, cal(A)[|1|]s = cal(N)[|1|] = one$より，
  $cal(A)[|x|]s != cal(A)[|1|]s$.
...
#align(center)[
  #table(
    ...
    [$b'$], [$b$],
    [$a_1 >= a_2$], [$not(a_1 <= a_2 and not(a_1 = a_2))$],
    [$a_1 < a_2$], [$a_1 <= a_2 and not(a_1 = a_2)$],
    [$a_1 > a_2$], [$not(a_1 <= a_2)$],
