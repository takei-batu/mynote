...
$
  cfg(S, s) -> s
$
直感的には状態$s$で文$S$の実行が停止し，その結果の状態が$s'$になるという意味になる．
$->$の定義は以下の規則で与えられる．
...
つまり，
$b$が$ff$に評価されるならば，
その状態を変えないまま$mono("while")$構文の実行を停止する．
...
状態$s$での実行$S$が
- *停止する*$<=> trans(S, s, s')$となる状態$s'$が存在する．
- *ループする*$<=> trans(S, s, s')$となる状態$s'$が存在しない．
- *常に停止する*$<=>$ 任意の$s$に対して$trans(S, s, s')$となる状態$s'$が存在する．
- *常にループする*$<=>$ 任意の$s$に対して$trans(S, s, s')$となる状態$s'$が存在しない．

#Ex[
  各文を$S$とする．
  - 常に停止するわけでも、常にループするわけでもない．
    - $s vx = zero$となる$s$に対して$trans(S, s, s')$となる状態$s'$が存在しないので，常に停止しない（厳密な反証は省略）．
...
#align(center)[
  #table(
    ...
    [[$while2$]], [$trans2(vwhile b vdo S, s, cfg(vif b vthen (S;vwhile b vdo S) velse vskip, s))$],
  )
]
...
状態$s$における文$S$の実行が
- *停止する*：$cfg(S, s)$で始まる有限導出シークエントが存在する
- *ループする*：$cfg(S, s)$で始まる無限導出シークエントが存在する

$cfg(S, s) =>^* s'$となる状態$s'$が存在するならば，
状態$s$における文$S$の実行が*正常に停止する*という．
$While$では$cfg(S, s)$がスタックすることがないため，
$S$の実行が*正常に停止する*ことと$S$の実行が*停止する*ことは同値である．
また，
任意の状態$s$に対して，
$S$が停止することを$S$は*常に停止する*といい，
$S$がループすることを$S$は*常にループする*という．

#Ex[
  以下の規則$["repeat"_"sos"]$を追加すればよい．
  $
    trans2(vrepeat S vuntil b, s, cfg(S\;vif b vthen vskip velse (vrepeat S vuntil b), s))
  $
]
...
任意の文$S$，
状態$s$，
計算状況$gamma, gamma'$に対して，
$
  [trans2(S, s, gamma) and trans2(S, s, gamma')] supset gamma = gamma'
$
が成り立つとき，
構造的操作意味論は*決定的*であるという．
...
#Ex[
  ...
  - 
    // 任意の状態$s$において$S$が停止する$and S$がループすることはない \
    $not$($S$が常に停止する$and S$が常にループする) \
    // $<=> not$($S$が常に停止する)$or not$($S$が常にループする) \
    // $<=> $($S$が常に停止する)$=> not$($S$が常にループする) \
    // $<=> $($S$が常にループする)$=> not$($S$が常に停止する)
]
...
#Ex[
  ...
  + $cfg(S\;vskip, s) =>^* gamma$と仮定すると，
    ...
    以上より，
    $s$において$S\;vskip$が停止する$<=> S$が停止する がわかる．

  + $S_1 = vwhile b vdo S$, $S_2 = vif b vthen S;S_1 velse vskip$とする．
  
    $cfg(S_1, s) =>^* gamma$を仮定すると，
    ある自然数$k$が存在して，
    $cfg(S_1, s) =>^k gamma$となる
    （ただし1ステップで$cfg(S_1, s)$が最終計算状況に遷移することはないため，
    $k > 1$である）．
    このとき，
    ある$S'$と$s''$が存在して，
    $
      cfg(S_1, s) => cfg(S', s'') =>^(k-1) gamma
    $
    $[while2]$より，
    $cfg(S_1, s) => cfg(S_2, s)$であるから，
    $S' = S_2, s'' = s$となる．
    よって，
    $cfg(S_2, s) =>^(k-1) gamma$であり，
    $cfg(S_2, s) =>^* gamma$がわかる．
    
    逆に$cfg(S_2, s) =>^* gamma$を仮定すると，
    ある自然数$k$が存在して，
    $cfg(S_2, s) =>^k gamma$となる．
    $[while2]$より，
    $cfg(S_1, s) => cfg(S_2, s)$であるから，
    $cfg(S_1, s) => cfg(S_2, s) =>^(k) gamma$を得る．
    よって，
    $cfg(S_1, s) =>^(k+1) gamma$であり，
    $cfg(S_1, s) =>^* gamma$がわかる．
    
    以上より，
    $s$において$S_1$が停止する$<=> S_2$が停止する がわかる．
  + 略
]

#Ex[]

=== The Semantic Function *$cal(S)_"sos"$*

$State$から$State$への部分関数を使って文の意味論が定められる：
$
  cal(S)_"sos" : Stm -> (State arrow.r.hook State)
$
これは，
$
  cal(S)_"sos" [|S|]s =
...
#Ex[
  ...
  // // $gamma$が最終計算状況のときは，
  // このとき，
  // $cfg(S_1, s) =>^* s'$となる$s$に対しては，
  // $cfg(S_2, s) =>^* s'$であるから，
  // $
  //   cal(S)_"sos" [|S_1|]s = s' = cal(S)_"sos" [|S_2|]s.
  // $
  // それ以外の$s$に対しては定義より，
  // $
  //   cal(S)_"sos" [|S_1|]s = "undef" = cal(S)_"sos" [|S_2|]s.
  // $
  // 以上より，
  // $cal(S)_"sos" [|S_1|] = cal(S)_"sos" [|S_2|]$.

  // 逆に，任意の状態$s$に対して$cal(S)_"sos" [|S_1|]s = cal(S)_"sos" [|S_2|]s$を仮定する．
  // このとき，
  // $gamma$を最終計算状況とし，
  // $cfg(S_1, s) =>^* gamma$を仮定すると，
  // $cal(S)_"sos" [|S_1|]s = gamma$である．
  // 仮定より，
  // $cal(S)_"sos" [|S_2|]s = gamma.$
  // このとき，
  // $cfg(S_2, s) =>^* gamma.$
  // よって，
  // $[cfg(S_1, s) =>^* gamma] => [cfg(S_2, s) =>^* gamma].$
  // 同様にして，
  // $[cfg(S_2, s) =>^* gamma] => [cfg(S_1, s) =>^* gamma].$
]
...
#Thm[
  $While$の任意の文$S$に対して，
  $cal(S)_"ns"[|S|] = cal(S)_"sos"[|S|].$
]

つまり，
- ある状態から実行した$S$が一方の意味で停止するならば，
  もう一方の意味でも停止し，
  その結果は等しくなる．
- ある状態から実行した$S$が一方の意味でループするならば，
  もう一方の意味でもループする．

#Lem[
  $While$の任意の文$S$と状態$s, s'$に対して，
  $
    cfg(S, s) -> s' supset cfg(S, s) =>^* s'
  $
]
...
#Lem[
  $While$の任意の文$S$と状態$s, s'$に対して，
  $
    cfg(S, s) =>^k s' supset cfg(S, s) -> s'
  $
]

定理2.26の証明は以下のようにして行われる．
任意の文$S$と状態$s$に対して，
$cal(S)_"ns" [|S|]s = s' supset cal(S)_"sos" [|S|]s = s'$およびその逆が補題2.27, 2.28より従う．
これより，$cal(S)_"ns" [|S|] = cal(S)_"sos" [|S|]$でなければならない．
すなわち一方が定義されているならば，
もう片方もそうであるし，
一方が定義されていないならば，
もう片方もそうである．
